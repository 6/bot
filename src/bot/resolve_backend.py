...
Usage:
    python -m bot.resolve_backend <backend>
...
def _runner_module(module_name: str) -> str:
    root = '${BOT_RUNNER_ROOT:-.}'
    return f'uv run --project "{root}" python -m bot.{module_name}'
...
def codex_backend(model: str, reasoning_effort: str, strength: str) -> dict:
    return {
        ...
        "cli": "codex",
        "setup_cmd": (
            f'{_runner_module("guard_backend_secrets")} '
            '--from-env CODEX_AUTH_JSON '
            'emit-masks && '
            f'{_runner_module("validate_codex_auth")} '
            '--from-env CODEX_AUTH_JSON '
            '--max-age-days 7 && '
        ...
        "run_cmd": (
            ...
            'STATUS=$?; '
            f'{_runner_module("agent_logs")} summarize '
            '"$AGENT_EVENTS_FILE" '
