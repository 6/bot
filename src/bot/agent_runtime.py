#!/usr/bin/env python3
"""Define canonical runtime paths for remote agent runs."""
...
def build_paths_from_root(profile: str, root: Path) -> dict[str, str]:
    return {
...
def runtime_root(profile: str) -> Path:
    ...
    return runner_temp / profile


def current_paths(profile: str) -> dict[str, str]:
    defaults = build_paths_from_root(profile, runtime_root(profile))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Emit canonical runtime paths for remote agent runs")
    parser.add_argument("profile", help="Runtime profile name")
    parser.add_argument("--runner-temp", type=Path, required=True)
    args = parser.parse_args()

    paths = build_paths(args.profile, args.runner_temp.resolve())
    ensure_dirs(paths)
